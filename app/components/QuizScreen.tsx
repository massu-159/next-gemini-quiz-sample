...
export default function QuizScreen({ questions, onComplete }: QuizScreenProps) {
  ...
  const getButtonClass = (index: number) => {
    ...
    if (index === questions[currentQuestion].correctAnswer) return 'bg-green-500 text-white';
        ...
        <div className="space-y-4">
          {questions[currentQuestion].choices.map((choice, index) => (
            <button
              ...
              className={`w-full p-4 rounded-lg text-left transition-all transform hover:scale-[1.01] active:scale-[0.99] ${getButtonClass(index)}`}
            >
              {choice}
            </button>
          ))}
